...
from ( 
    ...
    where
      dtm = '20230521_2023_all'
      and date_key<'2023-05-22'
    union all
    select
       date_key
      ,module
      ...
      ,0 as mkt_ecm_unclosed_channel_cash_cost
    from
      redcdm.dws_ads_industry_product_account_1d_di
    where
      dtm = '{{ds_nodash}}'
    union all
    -- 收入数据，来源crm
...
-----------------------------v2----------------------
insert overwrite table redcdm.dm_ads_industry_product_account_td_df partition(dtm = '{{ ds_nodash }}')
select 
   date_key
  ,module
  ,product
...
from ( 
    ...
    from
      redcdm.dm_ads_industry_product_account_td_df
    where
      dtm = '{{yesterday_ds_nodash}}' and (coalesce(module,'')<>'效果' or date_key<'2022-01-01' or date_key>='2023-01-01')--2022年效果数据需加market_target
    union all 
    select
       date_key
      ,module
