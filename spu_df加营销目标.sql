...
select campaign_id,spu_id,a.module,a.product,case
   ...
   end as market_target,a.marketing_target,min(date_key) as cam_dt,sum(case when date_key='{{ds}}' then cash_income_amt else 0 end) as cam_income_amt
...
group by campaign_id,spu_id,a.module,a.product,a.marketing_target,
case
     when a.marketing_target in (3, 8, 14,15) then '闭环电商广告'
     when a.marketing_target in (13) then '非闭环电商广告'
     when a.marketing_target in (2, 5, 9) then '线索广告'
...
select ----绑定spu的流水分摊 
  ...
  left join temp.temp_dm_ads_pub_spu_account_{{ds_nodash}} b on base.spu_id = b.spu_id
