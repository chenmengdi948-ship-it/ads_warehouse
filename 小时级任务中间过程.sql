...
--全量子账户产品以及当前和上小时
drop table if exists temp.dm_ads_rtb_advertiser_metric_hi_01{{ds_nodash}}_{{ts[11:13]}};
create table temp.dm_ads_rtb_advertiser_metric_hi_01{{ds_nodash}}_{{ts[11:13]}}
select
  virtual_seller_id,
  advertiser_id,
  hh,
...
from
  ...
  on 1=1
;
drop table if exists temp.dm_ads_rtb_advertiser_metric_hi_02{{ds_nodash}}_{{ts[11:13]}};
create table temp.dm_ads_rtb_advertiser_metric_hi_02{{ds_nodash}}_{{ts[11:13]}}
 --账户小时余额
  select
  ...
  group by
    ...
    2,3
    ;
drop table if exists temp.dm_ads_rtb_advertiser_metric_hi_03{{ds_nodash}}_{{ts[11:13]}};
create table temp.dm_ads_rtb_advertiser_metric_hi_03{{ds_nodash}}_{{ts[11:13]}}
 -- --账户离线日预算
  -- select granularity,
  --   advertiser_id,
  --   product,
...
from temp.dm_ads_rtb_advertiser_metric_hi_02{{ds_nodash}}_{{ts[11:13]}} t1 
full outer join 
(select *
from temp.dm_ads_rtb_advertiser_metric_hi_02{{ds_nodash}}_{{ts[11:13]}}
where after_hh is not null
)t2 on t1.hh=t2.after_hh and t1.virtual_seller_id=t2.virtual_seller_id

),
advertiser_budget_alias as (
...
from temp.dm_ads_rtb_advertiser_metric_hi_03{{ds_nodash}}_{{ts[11:13]}} t1 
full outer join 
(select *
from temp.dm_ads_rtb_advertiser_metric_hi_03{{ds_nodash}}_{{ts[11:13]}}
where after_hh is not null
...
from
  (
    select
      ...
      sum(account_day_budget) as budget_amt,
      sum(ystd_account_day_budget) as ystd_budget_amt,
      sum(account_day_budget_before_1d) as account_day_budget_before_1d
    from
      temp.dm_ads_rtb_advertiser_metric_hi_01{{ds_nodash}}_{{ts[11:13]}} t0
      inner join advertiser_cost_alias t1 on t0.advertiser_id = t1.advertiser_id
      and t0.product = t1.product
      and t0.market_target_type = t1.market_target_type
      left join 
      advertiser_budget_alias t2 on t0.advertiser_id = t2.advertiser_id
    ...
    from
      (
        ...
        from
          temp.dm_ads_rtb_advertiser_metric_hi_01{{ds_nodash}}_{{ts[11:13]}}
        group by
          1,
          2,
          3,
      ...
      left join advertiser_budget_alias t2 on t0.advertiser_id = t2.advertiser_id
      and t0.product = t2.product
      where tag = 2
    union all
    select
    ...
    from
      ...
      left join advertiser_balance_alias t1 on t0.virtual_seller_id = t1.virtual_seller_id
      and t0.hh = t1.hh
      left join 
      (select *
      from advertiser_budget_alias 
      where tag = 3
      )t2 on t0.advertiser_id = t2.advertiser_id
  ) t0
  left join advertiser_info_alias t1 on t0.virtual_seller_id = t1.virtual_seller_id
  where t0.hh<=(select max(hh) from advertiser_balance_alias)