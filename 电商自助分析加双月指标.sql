...
drop table if exists temp.temp_app_ads_industry_ecm_target_metric_di_{{ds_nodash}};
...
from
  ...
  join (
    ...
    where
      dtm = 'all'
      and day_dtm between f_getdate ('{{ds_nodash}}', -77) and '{{ds_nodash}}'
  ) dt on 1 = 1;
drop table if exists temp.temp_app_ads_industry_ecm_target_metric_di_{{ds_nodash}}_bimonth;

create table
  temp.temp_app_ads_industry_ecm_target_metric_di_{{ds_nodash}}_bimonth as
 --截止当日双月累计指标
    select
      date_key,
      dtm,
    ...
    select
      *
    from
      temp.temp_app_ads_industry_ecm_target_metric_di_{{ds_nodash}}
    where
      day_dtm >= f_getdate ('{{ds_nodash}}', -77)
    ) a
    left join ( --电商广告投放情况
    select
      dtm,
    ...
    where
      dtm between f_getdate ('{{ds_nodash}}', -77) and '{{ds_nodash}}'
    group by
      1,
      2
...
drop table if exists temp.temp_app_ads_industry_ecm_target_metric_di_{{ds_nodash}}_bimonth_02;

create table
  temp.temp_app_ads_industry_ecm_target_metric_di_{{ds_nodash}}_bimonth_02 as
 --截止当日双月累计指标
    select
      date_key,
      dtm,
    ...
    where
      marketing_target = 3
      and dtm between f_getdate ('{{ds_nodash}}', -77) and '{{ds_nodash}}'
    group by
      1,
      2
  ) b on a.brand_account_id = b.brand_account_id
...
drop table if exists temp.temp_app_ads_industry_ecm_target_metric_di_{{ds_nodash}}_bimonth_03;

create table
  temp.temp_app_ads_industry_ecm_target_metric_di_{{ds_nodash}}_bimonth_03 as
 --过去30天dgmv
 select
      date_key,
      dtm,
...
from
  (
    ...
    from
      temp.temp_app_ads_industry_ecm_target_metric_di_{{ds_nodash}}
    where
      day_dtm >= f_getdate ('{{ds_nodash}}', -15)
  ) a
  left join ( --电商广告投放情况
    select
      dtm,
    ...
    from
      redcdm.dm_ads_rtb_creativity_1d_di
    where
      dtm between f_getdate ('{{ds_nodash}}', -15) and '{{ds_nodash}}'
    group by
      1,
      2
  ) b on a.brand_account_id = b.brand_account_id
  ...
  left join ( --过去30天dgmv
    ...
    select
     ...
     dianbo_deal_gmv_2m
    from
     temp.temp_app_ads_industry_ecm_target_metric_di_{{ds_nodash}}_bimonth_03
  ...
  left join ( --品牌种草+转化
    ...
    select
      ...
      brand_account_id,
       brand_ti_user_cost,
       brand_ti_target_cost,--品牌种草人群定向
       brand_ti_user_cost_2m,
       brand_ti_target_cost_2m--品牌种草人群定向
    from
      temp.temp_app_ads_industry_ecm_target_metric_di_{{ds_nodash}}_bimonth_02
    where dtm between f_getdate ('{{ds_nodash}}', -15) and '{{ds_nodash}}'
  ...
  left join ( --双月收入
    ...
    select
      ...
      dplx_cash_cost_2m
  from
    temp.temp_app_ads_industry_ecm_target_metric_di_{{ds_nodash}}_bimonth
  where dtm between f_getdate ('{{ds_nodash}}', -15) and '{{ds_nodash}}'
  ) t1 on a.brand_account_id = t1.brand_account_id
  and a.day_dtm = t1.dtm
