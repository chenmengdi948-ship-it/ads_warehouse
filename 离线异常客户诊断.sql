...
---------非闭环电商-----------
drop table
  if exists temp.temp_app_ads_insight_advertiser_product_diagnosis_info_df_{{ds_nodash}}_ecm;

create table
  temp.temp_app_ads_insight_advertiser_product_diagnosis_info_df_{{ds_nodash}}_ecm as
select date_key,
  brand_account_id,
  advertiser_id,
  v_seller_id,
...
---------预算---------
drop table
  if exists temp.temp_app_ads_insight_advertiser_product_diagnosis_info_df_{{ds_nodash}}_budget;

create table
  temp.temp_app_ads_insight_advertiser_product_diagnosis_info_df_{{ds_nodash}}_budget as
select
  f_getdate(dtm) as date_key,
  '效果' as module,
  case
...
--------------base_metric-------------
drop table
  if exists temp.temp_app_ads_insight_advertiser_product_diagnosis_info_df_{{ds_nodash}};

create table
  temp.temp_app_ads_insight_advertiser_product_diagnosis_info_df_{{ds_nodash}} as
SELECT
  date_key,
  ...
  sum(
    case
      when module in ('效果', '品牌', '薯条') then cash_income_amt
      else 0
    end
  ...
  sum(
    case
      when module in ('品牌') then cash_income_amt
      else 0
    end
...
------当日指标-----------
drop table
  if exists temp.temp_app_ads_insight_advertiser_product_diagnosis_info_df_{{ds_nodash}}_all;

create table
  temp.temp_app_ads_insight_advertiser_product_diagnosis_info_df_{{ds_nodash}}_all as
select date_key,
  brand_account_id,
  advertiser_id,
  v_seller_id,
  ...
  case when substring(date_key,1,4)=substring(add_months('{{ds}}',-12),1,4) and date_key<=add_months(date_key, -12) then 1 else 0 end as is_last_year,
...
from
  (
    select coalesce(t1.date_key, t2.date_key) as date_key,
      ...
      ecm_unclosed_rgmv
    from
      temp.temp_app_ads_insight_advertiser_product_diagnosis_info_df_{{ds_nodash}} t1
      full outer join temp.temp_app_ads_insight_advertiser_product_diagnosis_info_df_{{ds_nodash}}_ecm t2 on t1.date_key = t2.date_key
  ...
  full outer join temp.temp_app_ads_insight_advertiser_product_diagnosis_info_df_{{ds_nodash}}_budget t3 on t1.date_key = t3.date_key
  ...
  ;
drop table
  if exists temp.temp_app_ads_insight_advertiser_product_diagnosis_info_df_{{ds_nodash}}_detail;

create table
  temp.temp_app_ads_insight_advertiser_product_diagnosis_info_df_{{ds_nodash}}_detail as
select brand_account_id_1 as brand_account_id,
  ...
  CASE
    WHEN (t1.module IN ('效果', '薯条', '品合', '内容加热'))
      OR (t1.module IS NULL)
  ...
  planner_name,
----redbi使用赛道逻辑
case when track_detail_name='其他' and if(module='品牌',brand_direct_sales_dept4_name,cpc_direct_sales_dept4_name)='美妆洗护行业' then '美妆' 
...
from temp.temp_app_ads_insight_advertiser_product_diagnosis_info_df_{{ds_nodash}}_all 
...
--------------------本双月----------------
  ...
  cash_income_amt_before_1y
from 
temp.temp_app_ads_insight_advertiser_product_diagnosis_info_df_{{ds_nodash}}_detail t1 
left join 
(
...
from 
  temp.temp_app_ads_insight_advertiser_product_diagnosis_info_df_{{ds_nodash}}_detail
...
from temp.temp_app_ads_insight_advertiser_product_diagnosis_info_df_{{ds_nodash}}_detail
